    ...
    {items.map((item, idx) => (
      <div
        ...
        <Image
          ...
          src={item.image}
          alt={item.name}
          className="object-fit !m-0 p-2 object-top rounded-full h-14 w-14 border-2 group-hover:scale-105 group-hover:z-30 border-primary bg-white  relative transition duration-500" />
      </div>
    ))}
  </>);
};
